...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ========== AUTH MODAL (defined early so we can use it) ==========
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            // Reload to show logged-in state on account page
    ...
    // ========== CHECK LOGIN – IF NOT, SHOW MODAL ==========
    if (!isLoggedIn()) {
        // Show modal
        openAuthModal();

        // If user closes modal, redirect to homepage
        const redirectToHome = () => {
            window.location.href = '../index.html';
        };
        closeAuthModal.addEventListener('click', redirectToHome, { once: true });
        ...
        // Stop further execution – page will reload after login or redirect on close
        return;
    }
    ...
    // ========== INITIALIZE (only runs if logged in) ==========
    initMobileNav();
    initUserDropdown();
    initLogout();
